...
const char *pWeaponSoundCategories[NUM_SHOOT_SOUND_TYPES] = { "empty", "single_shot", "single_shot_npc", "double_shot", "double_shot_npc", "burst", "reload", "reload_npc", "melee_miss", "melee_hit", "melee_hit_world", "special1", "special2", "special3", "taunt", "deploy" };
#else
...
KeyValues *ReadEncryptedKVFile( IFileSystem *pFilesystem, const char *szFilenameWithoutExtension, const unsigned char *pICEKey, bool bForceReadEncryptedFile /*= false*/, bool bUseJSON )
{
    ...
    if ( bUseJSON )
    {
        Q_snprintf( szFullName, sizeof( szFullName ), "%s.json", szFilenameWithoutExtension );
        CUtlBuffer buf{};
        if ( !pFilesystem->ReadFile( szFullName, pSearchPath, buf ) )
        {
            Warning( "Couldn't read script '%s' in search path '%s'\n", szFullName, pSearchPath );
            return NULL;
        }

        KeyValuesJSONParser parser{ buf };
        pKV = parser.ParseFile();
        if ( pKV == NULL )
        {
            Warning( "JSON error in '%s':%d: '%s'\n", szFullName, parser.m_nLine, parser.m_szErrMsg );
            return NULL;
        }
    }
    else
    {
        ...
        pKV = new KeyValues( "WeaponDatafile" );
        if ( !pKV->LoadFromFile( pFilesystem, szFullName, pSearchPath ) )
        {
            Warning( "Couldn't read script '%s' in search path '%s'\n", szFullName, pSearchPath );
            return NULL;
        }
    ...
    if ( bForceReadEncryptedFile ) // try to load the normal .txt file first
    {
#ifndef _XBOX
        if ( pICEKey )
        {
...
bool ReadWeaponDataFromFileForSlot( IFileSystem *pFilesystem, const char *szWeaponName, WEAPON_FILE_INFO_HANDLE *phandle, const unsigned char *pICEKey )
{
    ...
    *phandle = FindWeaponInfoSlot( szWeaponName );
    ...
    {
        pFileInfo->ErrorValues( szWeaponName );
    }
...
void FileWeaponInfo_t::Parse( KeyValues *pKeyValuesData, const char *szWeaponName )
{
    ...
    if ( iSlot >= MAX_WEAPON_SLOTS || iPosition >= MAX_WEAPON_POSITIONS )
    {
...
void FileWeaponInfo_t::ErrorValues( const char *szWeaponName )
{
    bParsedScript = false;
    bLoadedHudElements = false;
    ...
    Q_strncpy( szViewModel, pszNull, MAX_WEAPON_STRING );
    Q_strncpy( szWorldModel, pszNull, MAX_WEAPON_STRING );
    Q_strncpy( szAnimationPrefix, pszNull, MAX_WEAPON_PREFIX );
    iSlot = 0;
    iPosition = 0;
    iMaxClip1 = 0;
    iMaxClip2 = 0;
    ...
    m_bAllowFlipping = true;
    m_bMeleeWeapon = false;
    Q_strncpy( szAmmo1, pszNull, sizeof( szAmmo1 ) );
    iAmmoType = 0;
    Q_strncpy( szAmmo2, pszNull, sizeof( szAmmo2 ) );
    iAmmo2Type = 0;
    memset( aShootSounds, 0, sizeof( aShootSounds ) );
}